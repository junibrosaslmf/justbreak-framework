...
import PhoneNumberMenuItem from "./PhoneNumberMenuItem";
...
const FlagIconStyled = styled(PhoneNumberMenuItem)({
  width: 16,
  marginRight: 8,
});

const NativeSelectStyled = styled(Select)({
  width: 30,
  height: 30,
  padding: 8,
});
...
class PhoneNumber extends React.Component {
  ...
  guessSelectedCountry = memoize(
    (inputNumber, onlyCountries, defaultCountry) => {
      const secondBestGuess =
        onlyCountries.find((item) => item.iso2 === defaultCountry) || {};
  ...
  constructor(props) {
    ...
    if (inputNumber.length > 1) {
      ...
      // Default country
      countryGuess =
        onlyCountries.find((item) => item.iso2 === props.defaultCountry) || 0;
    } else {
      // Empty params
      countryGuess = 0;
    }

    const countryGuessIndex = countryData.allCountries.findIndex(
      (item) => item === countryGuess
    );
    const dialCode =
    ...
    const formattedNumber =
      inputNumber === "" && countryGuess === 0
        ...
        : this.formatNumber(
            ...
            this.getInputMask(countryGuess) ||
  ...
  updateDefaultCountry = (country) => {
    ...
    const newSelectedCountry = onlyCountries.find(
  ...
  handleInput = (e) => {
    ...
    if (e.target.value.length > 0) {
      ...
      formattedNumber = this.formatNumber(
        inputNumber,
        this.getInputMask(newSelectedCountry)
      );
    }
...
PhoneNumber.displayName = "MuiPhoneNumber";
